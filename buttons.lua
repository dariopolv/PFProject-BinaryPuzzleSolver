...
function gen(grid, var, position, length)
  bx = 500 + var 
  by = 60 + position
  w = bx + 20 
  ...
  for i, button in ipairs(grid) do
    ...
    hot = mx > bx and mx < bx + button_width + 100 + length  and
...
function generate_buttons(grid, position) 
...
function draw_buttons(a, b, var, position, length) 

  function love.draw()

    drawTables(a, b)
    gen(buttons2,var, position, length)
  end
end
